<div class="modal fade" id="ModalDeleteAllMAC" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Elimina tutti i MAC</h5>
            ...
            <form action="database/database.php" method="POST">
                <div class="modal-body">
                    ...
                    <input type="hidden" class="form-control" name="action" value="delete_all_mac" readonly>
                    <?php } else { ?>
                    <input type="hidden" class="form-control" name="action" value="request_delete_all_mac" readonly>
                    <?php } ?>
                    ...
                    <?php 
                        if($_SESSION['Ruolo'] == "Administrator") {
                            echo _("Vuoi davvero eliminare tutti i MAC?");
                        } else {
                            $idPlant = $_GET['id'];
                            $query = "SELECT * FROM `richieste_append` WHERE `IdImpianto_FK` = $idPlant AND `TabellaRichiesta` = 'mac'";
